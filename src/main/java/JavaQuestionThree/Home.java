...
public class Home {
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> parents = graph.getParents(graph.getDependancyGraph(),2);
		homeObject.Display(parents);

		// Using getAncestors(int id)
		System.out.println("-----------------------------------------");
		System.out.println("Ancestors of Node ");
		ArrayList<Integer> Ancestors = graph.getAncestors(graph.getDependancyGraph(),2);
		...
		ArrayList<Integer> children = graph.getChildren(graph.getDependancyGraph(),1);
		homeObject.Display(children);

		// Using getDescendants(int id)
		System.out.println("-----------------------------------------");
		System.out.println("Descendants");
		ArrayList<Integer> descendants = graph.getDescendents(graph.getDependancyGraph(),1);
		homeObject.Display(descendants);


		// Using deleteDependency(int id)
		graph.deleteDependency(graph.getDependancyGraph(),4);
		
		// Using deleteDependency(int parent, int children)
		graph.deleteDependency(graph.getDependancyGraph(),1,2);
